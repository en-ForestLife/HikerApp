...
void main() {
  final xmlData = '''<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
    ...
    &amp;lt;br /&amp;gt;&#xD;
    가라산 곁에 계룡산(鷄龍山:566m)·노자산(老子山: 565m)·앵산(鶯山:507m)·산방산(山芳山:507m)·선자산(扇子山:507m)·옥녀봉(玉女 峰:555m) 등 500m대 비탈산이 많아 농지가 적지만 바다로 둘려 수산물이 풍부하다. 계룡산∼가라산 종주(약 25㎞)보다 노자산∼가라산 쪽이 인기다. 두 산은 1cm 사이라는 거제 유머가 있다.&amp;lt;br /&amp;gt;&#xD;
    &amp;lt;br /&amp;gt;&#xD;
    봄이면 고로쇠 약수 채취로 붐비고 해양성기후에 잘 자라는 아열대식물인 동백나무·팔손이나무·소철·종려나무 등 600여 종이 우거졌다. 노자산의 천연기념물인 학동의 동백림(233)은 세계적인 팔색조(八色鳥:204) 번식지이며, 서불의 불로초 신화에 얽힌 신선산의 산삼 또한 손꼽는다. 거제 해금강이 보 이는 학동고개에서 오르면 벼늘［露積］바위ㆍ선녀바위가 있는 주능선 삼거리에 닿고, 다도해의 수묵화가 펼쳐진다. &amp;lt;br /&amp;gt;&#xD;
    &amp;lt;br /&amp;gt;&#xD;
  ...
  test('API TEST',(){
    ...
    var mountainData = <MountainModel>[];
    items.forEach((node) {
      mountainData.add(MountainModel.fromxml(node));
    });
    print(mountainData[0].mntninfohght);
  });
}
...
class MountainModel{
  ...
  factory MountainModel.fromxml(XmlElement xml){
